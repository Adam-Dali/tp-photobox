(() => {
  ...
  var init_config = __esm({
    "src/config.ts"() {
      API = "https://webetu.iutnc.univ-lorraine.fr/www/canals5/phox/api";
    }
  });
  ...
  function displayCategory(cat) {
    ...
    if (!el) return;
    el.textContent = `Cat\xE9gorie : ${cat.categorie.nom}`;
  }
  ...
  var import_handlebars, template;
  var init_ui = __esm({
    "src/ui.ts"() {
      import_handlebars = __toESM(require_handlebars());
      template = import_handlebars.default.compile(`
<img src="{{img}}" />
<h3>{{titre}}</h3>
<p>{{descr}}</p>
<p>{{type}} - {{width}}x{{height}}</p>

<h4 id="cat">Cat\xE9gorie :</h4>
<h4>Commentaires :</h4>
<ul id="comments"></ul>
`);
    }
  ...
  var require_index = __commonJS({
    "src/index.ts"() {
      ...
      function getPicture(id2) {
        loadPicture(id2).then((photo) => {
          displayPicture(photo);
          return Promise.all([
            loadResource(
              "https://webetu.iutnc.univ-lorraine.fr" + photo.links.categorie.href
            ).then(displayCategory),
            loadResource(
              "https://webetu.iutnc.univ-lorraine.fr" + photo.links.comments.href
